module Data.Context.Base where
...
module WeakenOps {ℓ} (T : Pred ℕ ℓ) (ext : Extension T) where
  ...
  roundtripₗ : ∀ {i} → T (toℕ (Data.Fin.fromℕ i)) → T i
  roundtripₗ {i} x rewrite toℕ-fromℕ i = x

  roundtripᵣ : ∀ {i} → T i → T (toℕ (Data.Fin.fromℕ i))
  roundtripᵣ {i} x rewrite toℕ-fromℕ i = x

  insertUnder : ∀ {n : ℕ} → Ctx T n → (i : Fin (suc n)) →
    T (toℕ (opposite i)) → Ctx T (suc n)
  insertUnder {n} Γ zero x = roundtripₗ x ∷ Γ
  insertUnder {n} (t ∷ Γ) (suc i) x rewrite opposite-suc i =
    weaken t ∷ insertUnder Γ i x
